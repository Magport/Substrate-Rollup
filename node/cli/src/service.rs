...
pub fn create_extrinsic(
	...
	function: impl Into<rollup_runtime::RuntimeCall>,
	nonce: Option<u32>,
) -> rollup_runtime::UncheckedExtrinsic {
	...
	let period = rollup_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: rollup_runtime::SignedExtra =
		(
			frame_system::CheckNonZeroSender::<rollup_runtime::Runtime>::new(),
			frame_system::CheckSpecVersion::<rollup_runtime::Runtime>::new(),
			frame_system::CheckTxVersion::<rollup_runtime::Runtime>::new(),
			frame_system::CheckGenesis::<rollup_runtime::Runtime>::new(),
			frame_system::CheckEra::<rollup_runtime::Runtime>::from(generic::Era::mortal(
				period,
				best_block.saturated_into(),
			)),
			frame_system::CheckNonce::<rollup_runtime::Runtime>::from(nonce),
			frame_system::CheckWeight::<rollup_runtime::Runtime>::new(),
			pallet_skip_feeless_payment::SkipCheckIfFeeless::from(
				pallet_asset_conversion_tx_payment::ChargeAssetTxPayment::<
					rollup_runtime::Runtime,
				>::from(tip, None),
			),
		);

	let raw_payload = rollup_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			rollup_runtime::VERSION.spec_version,
			rollup_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	rollup_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		rollup_runtime::Signature::Sr25519(signature),
		extra,
	)
}
